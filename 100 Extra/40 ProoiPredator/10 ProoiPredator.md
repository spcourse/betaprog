...
Als een variabele of lijst in meerdere functies in een programma gebruikt wordt is het soms handig om die variabel als zogenaamd 'globaal' te definieren. De functies in het programma kunnen dan aangeven dat ze die variabelen willen gebruiken en kunnen die dan ook aanpassen. Deze variabelen worden daarmee gedeeld. In ons stukje code zijn dat twee lijsten: `posities_konijnen_x` en `posities_konijnen_y`. Deze lijsten worden namelijk gebruikt in elk van de drie functies: de startwaardes worden gedefinieerd in `prooipredator()`, ze worden veranderd in de functie `verplaats_de_konijnen()` en daarna getekend in `teken_het_bos()`. Deze syntax is als volgt: de lijsten worden eerst in het hoofdprogramma (helemaal bovenaan) als globaal gedefinieerd en vervolgens wordt aan het begin van elk van de drie functies vermeld dat we ze willen gebruiken.
...
We hebben hiermee nu de mogelijkheid om bij de start van de simulatie elk konijn een unieke bewegingsrichting te geven. Voer bovenstaande veranderingen in in de code, geef elk van de konijnen een eigen richting en laat de simulatie 20 seconden lopen in plaats van 10. 
